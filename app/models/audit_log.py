...
class AuditLog(db.Model):
    ...
    def get_action_color(self):
        """Retorna uma cor de badge para a ação do log"""
        action = (self.action or "").lower()
        if "criou" in action or "create" in action:
            return "success"
        elif "editou" in action or "update" in action or "alterou" in action:
            return "warning"
        elif "removeu" in action or "delete" in action or "apagou" in action:
            return "danger"
        else:
            return "secondary"

    def get_action_display(self):
        """Retorna uma string amigável para a ação"""
        action = (self.action or "").lower()
        if "criou" in action or "create" in action:
            return "Criação"
        elif "editou" in action or "update" in action or "alterou" in action:
